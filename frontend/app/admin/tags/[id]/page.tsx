...
import { api, TagAdmin } from '@/lib/api';
...
function EditTagForm() {
  ...
  useEffect(() => {
    api.getTagAdmin(id)
      .then((tag: TagAdmin) => {
        const fr = tag.translations.find(t => t.lang === 'fr');
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <Link href="/admin/tags" className="text-sm text-slate-500 hover:text-primary">← Retour aux tags</Link>
          <h1 className="text-3xl font-serif font-bold mt-2">Modifier le tag</h1>
        </div>
